...
module "deep_research_lambda_role" {
  source = "../../base-infra/iam-lambda"

  role_name         = "${var.project_name}-${var.environment}-deep-research-role"
...
module "lambda" {
  for_each = {
    ...
    if k != "deep-research-state-machine-executor" # Exclude the special executor lambda
  }
  ...
  lambda_role_arn        = module.deep_research_lambda_role.role_arn
...
# 3. State Machine
###############################################################################
module "deep_research_state_machine" {
  source = "../../base-infra/step-function"

  project_name       = var.project_name
  environment        = var.environment
  state_machine_name = "deep-research-workflow"
  state_machine_type = "STANDARD"
  definition = templatefile("${path.module}/state-machine.tftpl", {
    deep_research_lambda_arn       = module.lambda["deep-research"].lambda_arn
    deep_research_prompt_lambda_arn = module.lambda["deep-research-prompt"].lambda_arn
    deep_research_query_lambda_arn  = module.lambda["deep-research-query"].lambda_arn
  })
}
...
module "state_machine_executor_lambda" {
  source        = "../../base-infra/lambda"
  function_name = "blackbox-${var.environment}-deep-research-state-machine-executor"

  # Configuration from the .tfvars file
  runtime       = var.lambdas["deep-research-state-machine-executor"].runtime
  timeout       = var.lambdas["deep-research-state-machine-executor"].timeout
  memory_size   = var.lambdas["deep-research-state-machine-executor"].memory_size
  layers        = [for layer_key in var.lambdas["deep-research-state-machine-executor"].layers : var.available_layer_arns[layer_key]]

  # Inject the Step Function ARN into this specific Lambda
  environment_variables = {
    DEEP_RESEARCH_STEP_FUNCTION_ARN = module.deep_research_state_machine.state_machine_arn
    SSM_PREFIX                      = "blackbox-${var.environment}"
  }
